...
const Hotelcard = ({room, index}) => {
  return (
      <Link to={'/rooms/' + room._id} onClick={() => scrollTo(0,0)} key={room._id}>
        <img src={room.images[0]} alt="" className='relative max-w-70 w-full rounded-xl overflow-hidden bg-white text-gray-500/90 shadow-[0px_4px_8px_0px_rgba(0,0,0,0.5)]'/>
    ...
    <div className='p-4 pt-5'>
        ...
        <button className='px-4 py-2 text-sm font-medium border border-gray-300 rounded hover:bg-amber-50 transition-all cursor-pointer'>Book Now</button>
        </div>
    </div>
      </Link>
  )
